...
> B 站和知识星球都没有为本项目提供稳定的官方采集接口。本项目复用管理员已登录的网页会话，接口可能因协议变化、风控或平台规则而不可用；它不会绕过验证码、限流、登录或内容权限。知识星球要求从浏览器请求中导入 access token，采集端按网页 Cookie 合同请求；若上游返回业务码 `1059`，管理台会提示重新导入 Session 或检查协议变化。请只归档你有权访问和保存的内容。
