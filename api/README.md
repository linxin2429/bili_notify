...
```sh
npx --yes @redocly/cli@2.9.0 lint api/openapi.yaml --config api/redocly.yaml
```

Generate deterministic TypeScript transport types when wiring the UI client:

```sh
npx --yes openapi-typescript@7.10.1 api/openapi.yaml --output web/ui/src/shared/api/generated/schema.d.ts
```
