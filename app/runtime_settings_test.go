...
func TestRuntimeSettingsManagerUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "success", mutate: func(settings *model.RuntimeSettings) {
			settings.BilibiliDynamicIntervalSec = 45
		}},
		...
		{name: "persistence failure", mutate: func(settings *model.RuntimeSettings) {
			settings.BilibiliDynamicIntervalSec = 60
		}, closeDB: true, wantError: true},
	}
