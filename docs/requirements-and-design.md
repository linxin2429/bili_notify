...
代码按功能划分为顶层包：`bilibili` 负责 B 站网页接口、二维码登录以及协议结果到中立 `Content/Attachment` 的转换，`zsxq` 负责知识星球 Session 导入、严格解析、动态回补和评论同步，`platform` 保存 composition root 使用的 `Module/PlatformMeta` 契约，`media` 负责安全附件本地化，`state` 负责事务和持久化，`notify` 负责投递协议，`service` 负责编排 B 站、Outbox、OAuth 和领域事件，`web` 负责认证、`/api/v4`、WebSocket 与嵌入式管理台。`app` 从模块表启动各平台 Runner；平台特有的 feed、relation、回补和登录方式留在 adapter。知识星球客户端按请求显式传 token，不持有共享 cookie jar；协议与安全边界详见 `docs/zsxq-session-api-design.md`。
...
跨平台持久化以平台作为所有身份的一部分：账号写入 `platform_accounts`；来源 ID 为 `bilibili:up:{uid}` 或 `zsxq:planet:{group_id}`；知识星球来源还保存 `all | selected_authors` 模式及用户 ID/可选显示名称列表；内容 ID 为 `{platform}:content:{external_id}`；评论 ID 为 `{platform}:comment:{external_id}`。用户 ID 是匹配依据，名称只是管理员备注且不会从主题自动改写。`contents` 保存当前正文、类型、统计、首次发现、最后同步和删除 tombstone，`attachments` 保存远端元数据与本地相对路径，`comment_nodes` 使用邻接表保存 `root_id/parent_id`，查询时稳定重建 `children`。内容/评论归档、seen 与 Outbox 在同一事务中提交；Outbox 只接受 `content | comment | ai | system` 四种中立、创建时密封的载荷，发送时不得从可变事实表重建，`Dynamic` 只属于 B 站协议域。系统告警直接写 `system`，不创建伪内容。禁用渠道会在同一事务把待投递项转成 `suspended`，再次启用时恢复为立即可领取的 `pending`，因此不占用健康渠道的 due 批次。编辑只更新当前快照，完整成功的评论分页才能把缺失节点标记为删除，恢复已有 ID 不生成新通知。一轮同步把同一内容的多个目标作者评论合并为一个 `comment_digest`，载荷只包含新触发节点及各自从已知根开始的祖先路径。

轮询器默认以 30 秒为目标周期、每个平台 2 请求/秒、4 个同时在途请求。B 站和知识星球分别拥有一个覆盖该平台全部 HTTP 路径（包括媒体）的 RequestGate，统一执行速率、并发、请求超时和风控暂停；Gate 的并发额度从请求接纳持续到响应 Body 关闭，热更新 gate 时先停止接纳并等待旧请求排空，再安装新约束。运行设置先由所有运行态消费者接受，再持久化；任一步失败会把已应用的 logger/gate/runner 设置恢复到旧值，不允许静默产生数据库与运行态分裂。服务默认每 10 分钟批量查询当前登录账号与监控 UP 的关注关系：已关注且完成空间同步的 UP 使用账号综合动态流，其余 UP 轮询空间动态；关系未知时明确走空间接口。已关注 UP 默认每 30 分钟额外执行一次空间完整性校验。空间和综合流默认最多翻 10 页；超过上限不会静默推进游标，综合流会退回空间同步后重新建立基线。采集从不读取渠道来决定是否运行：零启用渠道时仍归档、写 seen、推进基线和同步评论编辑/删除，只是不创建 Outbox。
