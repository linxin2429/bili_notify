...
轮询器默认以 30 秒为目标周期、每个平台 2 请求/秒、4 个同时在途请求。B 站和知识星球分别拥有一个覆盖该平台全部 HTTP 路径的 RequestGate，统一执行速率、并发、请求超时和风控暂停；热更新 gate 时先停止接纳并等待旧请求排空，再安装新约束。服务默认每 10 分钟批量查询当前登录账号与监控 UP 的关注关系：已关注且完成空间同步的 UP 使用账号综合动态流，其余 UP 轮询空间动态；关系未知时明确走空间接口。已关注 UP 默认每 30 分钟额外执行一次空间完整性校验。空间和综合流每轮默认最多翻 10 页。空间流逐页原子保存原始条目与分页游标，页数是每轮工作预算，达到预算后下一轮继续，扫描阶段最长 30 秒；每次请求单独限时 10 秒。扫描保存开始时的 seen 边界及已有待处理条目边界，恢复或重扫时不把本次刚提交的内容误作历史终点。游标无效时保留已发现条目并退避重扫；综合流计数不足、游标异常或超过预算时保留已发现条目，退回各 UP 独立的空间补采，避免异常卡片持续阻塞账号综合流。空间和综合流不会同时采集同一个 UP。采集从不读取渠道来决定是否运行：零启用渠道时仍归档、写 seen、推进基线和同步评论编辑/删除，只是不创建 Outbox。

评论监控按平台独立调度。B 站默认每 120 秒同步每个 UP 最近 N 条（默认 10）可映射评论区坐标的内容。每轮翻完全部根评论以读取各楼 `rcount` 与根列表预览；首次成功同步建立基线并展开每个有子回复的根，此后仅当根是新楼、已归档存活子评数量与 `rcount` 不一致、或根列表预览出现未知 `rpid` 时才请求该根的子回复。未展开且仍出现在普查中的楼保留已归档子树，不因跳过子评请求而标记删除或把树标为不完整；根评论消失且普查完整时才 tombstone。根普查或必要子评翻页被截断时整轮不提交，以免先写入触发节点、重试补祖先时不再通知。空评论区的完整普查是合法基线，之后出现的新回复仍要通知。不根据根列表顺序提前停翻，也不用 `acount` 作为完整性依据。退出最近 N 窗口后保留树但不再请求。知识星球默认每 600 秒同步全部已归档主题：主题详情中的 `show_comments` 节点数与 `comments_count` 一致时，该快照可直接作为完整评论树；不一致时必须继续翻完独立评论接口，不能把展示子集误判为完整树。主题内文件只有元数据，下载前通过 `/v2/files/{file_id}/download_url` 取得临时地址；禁止下载或文件已删除时仍归档元数据，鉴权、限流和上游故障则让整轮重试。上一轮未结束时不启动重叠任务。两个平台首次同步评论均只建立基线。运行设置由 SQLite 持久化并热更新；环境变量只为空库播种，B 站和知识星球分别使用 `BILI_NOTIFY_BILIBILI_*` 与 `BILI_NOTIFY_ZSXQ_*`。

知识星球匿名提问不会返回提问者身份；归档时使用主题 ID 构造稳定的匿名作者 ID，不能因为缺少 `question.owner` 阻断整页同步。知识星球业务码 `1059` 表示当前请求协议被上游拒绝，必须作为独立、可行动的错误展示，不能伪装成普通网络故障、权限错误或响应结构变化。采集复用管理员导入的网页 access-token Cookie，采用已验证的网页 headers 和 `/v2/topics/{id}/info` 合同，不伪造移动 SDK 签名，也不通过浏览器自动化或第三方代理绕过登录和权限边界。

同一待处理批次的新内容按发布时间和稳定 ID 由旧到新处理；跨轮补采和失败重试不承诺全局通知顺序。知识星球启用时先固定最新页高水位，再持久化分页回补水位以前的历史；增量轮询持续翻页直至旧高水位，确保指定作者不在首页时仍可发现。回补内容不通知，回补期间水位以后的新内容走实时通知。修改知识星球作者范围只影响之后扫描到的主题：不删除已有档案、不重置高水位或回补游标，也不补采已经越过的过滤内容；未完成回补从当前游标立即采用新范围。任务只有在平台 HTTP 状态和业务码均成功后才删除；网络错误、429 和 5xx 分级重试，不可恢复配置或鉴权错误进入阻塞。数据库 v13 新增采集待处理条目、空间扫描检查点和综合流未归属条目表，不修改历史内容。删除来源会级联清除其采集重试与检查点，并通过索引删除所属 Outbox，并在同一数据库事务中级联删除内容、附件元数据、评论、seen 和同步状态，同时写入持久化媒体清理任务；后台 cleaner 幂等执行并持久化重试，进程重启不会丢失清理意图。删除后的知识星球来源不会因登录或更换 token 自动恢复，需要管理员手动重新添加。

## 3. B站与通知协议

B站适配器使用二维码生成、二维码轮询、导航验证、关注关系 `/x/relation/relations`、综合动态更新 `/x/polymer/web-dynamic/v1/feed/all/update`、综合动态 `/x/polymer/web-dynamic/v1/feed/all`、空间动态、图文详情 `/x/polymer/web-dynamic/v1/opus/detail`，以及评论根列表 `/x/v2/reply` 与子评论 `/x/v2/reply/reply`。动态请求显式携带登录 Cookie 与 `itemOpusStyle,listOnlyfans,onlyfansAssetsV2` 功能开关，以获取账号有权查看的充电动态。综合流只深度解析已关注的监控 UP；原始条目先持久化到 `collection_items`，持久化成功后才允许推进账号级 `update_baseline`；崩溃重放按来源和动态 ID 幂等。每条动态的档案、seen、Outbox 与待处理条目删除原子提交。缺失作者 UID 的综合流条目存入 `collection_feed_gaps` 供诊断，并触发所有受影响 UP 的空间校验，其他条目继续处理。登录成功后必须再次验证账号 UID 和名称才加密保存 Cookie；切换账号会重建关注关系和综合流基线，但保留内容档案与 per-UP 去重状态。不配置风控规避逻辑，不实现 WBI（若经典评论接口全面失效需另立项）。二维码状态由 Engine 每两秒推进并通过领域事件推送到管理台。 异常综合流原文仅作为诊断样本，全库按创建时间及插入顺序保留最近 100 条；实际补采依靠空间扫描检查点。v13 同时为样本创建查询索引和账号写入触发器：B 站首次登录清理其他账号样本，切换账号或删除账号清理全部样本，同账号续期及其他平台操作保留样本，清理与账号变更原子提交。

B 站登录默认自动续期，不增加管理台设置或公共 API。启动恢复和扫码成功后立即检查，此后每 10 分钟维护一次；以 `/x/passport-login/web/cookie/info` 返回的 `refresh` 标志判断续期时机，使用服务端毫秒时间戳生成 RSA-OAEP/SHA-256 CorrespondPath，通过网页获取 refresh CSRF，再交换新 Cookie 和 refresh token。`bilibili.SessionClient` 只处理显式会话快照和网络协议，不持有共享 Cookie jar，也不访问存储；`service/bilisession` 通过协议、存储小接口维护轮换流程和调度；Engine 负责生命周期、会话锁、状态事件及通知，`state` 负责加密持久化。

扫码响应的 refresh token 与 Cookie 一起存入带版本的加密载荷，读取兼容历史 Cookie map，无需修改数据库表结构。旧登录缺少刷新凭据时保留有效采集状态，每次服务运行或重新登录后最多提示一次重新扫码以启用续期。续期取得的新凭据必须校验 UID 与原账号一致；先原子保存新凭据和待确认的旧 refresh token，再切换采集客户端，最后以新 Cookie 和旧 token 确认更新。保存失败不切换、不确认；确认失败保留新会话和待确认信息，下轮及重启后优先补确认，成功后清除待确认信息。同账号续期不重置关注关系、内容去重或采集基线。

刷新、扫码提交、启动恢复与注销使用同一会话写锁，采集使用读锁；注销取消正在进行的扫码，迟到的扫码结果必须检查登录任务是否仍有效。每次维护在取得锁后最多 60 秒，各网络请求最多 10 秒。临时网络失败、限流和协议异常不直接标记登录失效，下轮重试并遵守风控暂停与 `Retry-After`；只有明确的登录校验拒绝才使会话失效，刷新凭据被拒绝单独提示重新扫码。续期网络请求禁止自动重定向，响应最多 1 MiB；日志仅记录阶段、结果、HTTP 状态和业务码，Cookie、刷新凭据、CSRF、CorrespondPath 和上游响应正文不进入日志或公共账号投影。加密载荷升级后不支持直接回退到只认识旧 Cookie map 的程序版本。
...
附件统一落在 `data_dir/media/{platform}/{source_id}/{content_id}/`。B 站继续只下载可安全识别的图片并维持 10 MiB 单图保护；每张图片使用独立的 10 秒截止时间，覆盖 DNS、重定向、响应头及响应体，超时保留远程 URL 并继续其他内容，不会因 CDN 保持连接而挂住采集；知识星球支持图片、文件、音频和视频，默认单件上限 500 MiB、平台总预算 50 GiB。超过单件限制或总预算时只保留元数据，不删除旧档案，也不阻断正文、评论和通知。所有下载只接受 HTTP(S) 公网目标，每次重定向重新校验；路径段由 `pathologize` 生成，临时文件为 `0600`，最终提交由 `fileflow` 执行冲突安全移动并持久化其实际返回路径，路径穿越和符号链接均拒绝。知识星球 access-token Cookie 只发送给原始 API 域名，跳转或 CDN 请求会剥离 Cookie 与 Authorization；签名下载 URL 和本地绝对路径不出现在日志、通知或列表响应。认证附件端点支持 Range，并设置 attachment disposition 与 `nosniff`。清理器不递归删除不可信目录，只删除任务指定文件并向上修剪 `media/` 内的空目录；越界或符号链接路径进入阻塞状态。

动态通知保存接口返回的完整正文，并按类型提取标题、简介、内容直达链接、富文本链接、封面或多图、视频时长与播放信息、互动统计和转发原文。正常解析的非专栏动态不为补充内容发起额外的 B 站请求。专栏详情请求前检查 `buvid3`；已保存的非空设备 Cookie 优先使用，否则通过 `/x/frontend/finger/spi` 正常初始化一次，缓存在客户端内存中，与账号凭据分别管理，已有会话的有效设备标识也纳入缓存，续期或重新登录不丢失、并发调用不重复初始化、等待可取消。匿名请求使用去除 Cookie Jar 的 HTTP 客户端副本，初始化请求不携带登录 Cookie；该匿名接口的 HTTP 401/403 或业务 -101/-111 归类为条目级临时错误，保留状态码但不使账号失效，失败交给现有条目退避重试；不旋转已有设备标识来规避风控，不新增配置或数据库迁移。详情响应即使 `code=0`，只要出现 `MODULE_TYPE_PAYWALL` 就按 `content_access` 错误保留待重试条目，不归档付费预览、不写 seen/Outbox、不使整个账号登录失效。`DYNAMIC_TYPE_ARTICLE` 及其转发原文的列表卡片只有截断摘要和封面，发现后必须再请求 `/x/polymer/web-dynamic/v1/opus/detail`（`htmlNewStyle`）取得完整段落和文中图片，再归档、本地化图片并入队；详情失败不得写该条目的 seen 或 Outbox，也不阻止同批正常动态提交。解析、详情或单条入库失败保留原始条目、初次发现时的基线语义、失败原因、尝试次数及下次重试时间；退避从 1 分钟倍增到最多 1 小时，重启或重复发现不重置退避。解析重试会尝试刷新空间首页同 ID 卡片；仍不可解析的载荷继续保留供后续协议修复，不能直接标记成功。综合流每轮先处理各 UP 的独立重试，再统一写入采集结果；共享请求错误或后续条目的认证/风控错误优先统一结算，避免单条失败和整批失败重复递增同一 UP 的失败计数。每个 UP 每轮最多处理 100 条，处理阶段最长 30 秒；成功条目独立更新评论跟踪目标，即使同批仍有失败。初始或充电基线的延迟重试继续只归档不通知。升级后每个已有 UP 首次可见的充电动态只归档并建立 seen 基线，不投递；同批普通新动态照常投递，后续新充电动态正常投递。评论通知在发现 UP 回复后按 root 展开对话串。邮件与 Microsoft Graph 在存在本地文件时以内联 CID/附件嵌入图片，否则退回远程 `<img src>`；钉钉自定义机器人继续使用可展示外链图片的 Markdown（`![](CDN URL)`，不用本地文件）；飞书应用机器人上传本地图片并在富文本中嵌入；企业微信先发正文 Markdown，再按序追加 `msgtype=image`（原始字节 ≤2 MiB）。机器人消息在各平台限制内按 UTF-8 边界截断，始终保留截断提示和原内容链接。
...
常规 CI 只在目标为 main 的 PR 和手动调用时运行。main 的 strict required check 要求合并候选基于最新 main 通过 `CI Gate`，因此合并后不再对等价代码重复执行常规 CI；CodeQL 仍在 main push 后运行，用于更新默认分支的安全扫描状态。PR CI 构建并冒烟验证 `linux/amd64` 主服务与 AI Worker 生产镜像：主服务验证 nonroot/只读运行、初始化、健康检查、优雅停止和持久化重启，Worker 通过共享 Unix Socket 调用真实 `GetCapabilities`，确认 nonroot/只读容器以及 yt-dlp、FFmpeg、gRPC 服务均可用；`CI Gate` 聚合所有结果并作为 main 的唯一 required check。现有 self-hosted 优先、macOS 次选、GitHub-hosted fallback 的 runner 选择策略保持不变。PR 不登录仓库、不发布镜像。独立 Release 入口接受 main 历史上的 `app/vMAJOR.MINOR.PATCH` 与 `worker/vMAJOR.MINOR.PATCH`，调用共用的组件发布 workflow；旧 `v*` 不再触发发布。两个组件各自维护版本及并发组，一个 tag 只处理对应组件。构建输入清单通过 Git tree 的路径、blob ID 和模式生成指纹，覆盖生产源码、嵌入资源、依赖及 Dockerfile；共享 gRPC 契约、`.dockerignore` 和发布实现影响两边。普通测试、文档及部署配置不计入发布输入，Worker 镜像只复制运行时包和生产依赖锁。仅测试变化仍经过 PR 的完整门禁。
...
B 站自动续期复用 `bili_notify.workflow.runs` / `bili_notify.workflow.duration`，固定标签为 `platform=bilibili`、`workflow=session_maintenance`，结果限定为 `not_due`、`renewed`、`confirmed`、`requires_login`、`error`、`skipped`、`canceled`。`renewed` 仅在新凭据落库、切换、上游确认和确认状态落库全部完成后记录；补做上次确认记为 `confirmed`，激活新 Cookie 但确认失败仍记为失败。该工作流不会更新采集最后成功时间。启动恢复和周期维护分别创建 `bilibili.session.restore` / `bilibili.session.maintain` span，续期协议的每次 HTTP 请求作为子 span 并复用 B 站请求计数/耗时；业务码拒绝、响应结构异常即使 HTTP 200 也记为请求失败。CorrespondPath 只使用 `/correspond/1/{correspondPath}` 固定模板作为 operation，指标和 trace 都不得包含真实路径、凭据、账号标识或上游错误正文。日志使用维护上下文，使 OTel log record 自动关联 trace/span；成功轮换与补确认记录 info，无需刷新和跳过记录 debug，失败与需扫码沿用 warn。Grafana 总览增加进程累计维护结果及一小时窗口的维护 p95 耗时，Logs and Traces 增加会话维护链路与日志面板；复用既有 Collector 和数据源，不引入新的观测服务或配置项。
...
Trace 用于关联管理 HTTP、采集/评论/关系/认证/投递/审计工作流、B 站逻辑外部操作和 GORM/SQLite。Outbox 创建任务时把当前采集 span 的 W3C `traceparent` 写入任务载荷，异步投递恢复它作为 `delivery.send` 的父上下文，使采集、SQLite 入队、投递和通知渠道调用出现在同一条 trace；同一内容的多渠道投递形成并列分支，重试继续使用最初的采集上下文。系统通知等没有有效来源上下文的任务归入当次投递调度 trace，非法上下文只降级、不影响投递。空闲投递轮询及其 SQLite 查询不产生 root span。对这个低流量服务使用 parent-based always-sample，不记录探针、静态资源和 WebSocket 长连接，只记录握手；外部请求 span 不记录完整 URL/查询；B 站 HTTP 成功但业务码非零也记为 error，详情 span 可记录 opus ID、HTTP 状态及业务码，不记录响应正文或凭据；GORM span 不记录查询变量。运行时导出失败不停止业务，非法 SDK/协议配置在启动时拒绝。异步父 span 可以先于投递子 span 结束，因此重试会拉长整条 trace；若跨度超过 Tempo 保留期，只能看到保留窗口内的部分。自动 AI 任务通过采集事务内的 savepoint 创建并保存当前 W3C `traceparent/tracestate`；配置读取、解密或建任务失败只回滚 AI 部分，内容事务同时保存独立的 `collection_items(kind=ai)` 调度意图（含内容和渠道快照及 trace 上下文）。AI 状态维护循环每 10 秒检查到期意图，按 1 分钟至 1 小时退避重试，成功建完整任务链后原子删除意图，失败输出可定位动态 ID 的日志，不依赖 B 站会话有效性。调度时恢复为 `ai.job.execute` 的父上下文，Go gRPC client 将上下文注入 Worker，使 Worker gRPC server span 与供应商 HTTP span 和动态采集属于同一 trace；总结任务继承转写执行 span，终态 AI 通知再继承当前 AI span。工作台任务则从创建任务的管理 API server span 开始，浏览器本身不接入 OTel。
...
- 空间动态与综合流的多页正常路径、seen 前沿停止、空或重复 offset、接口计数不足以及页数上限；不完整分页允许提交已持久化的正常内容，但必须保留未完成的扫描边界和游标；覆盖重启、同批故障隔离、退避不被重复发现重置，以及综合流回退后不会被刚写入的 seen 前沿提前截断；评论根页或子回复达到扫描上限时，已发现通知必须标记 `incomplete`；
- 投递调度通过受控阻塞渠道验证初始及热更新后的并发上限，通过五段退避及饱和边界验证重试，并保留不依赖绝对耗时门限的消息构造 benchmark；
- 各动态类型的富内容解析、专栏图文详情补全、评论区坐标映射、UP 回复发现与根串展开、基线、去重、Outbox、渠道渲染、篇幅边界、重试与通知协议；
