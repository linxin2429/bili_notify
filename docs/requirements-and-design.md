...
PR 与 main push 使用同一条 `linux/amd64` 生产镜像构建和冒烟测试路径：普通测试与观测配置验证并行完成后，镜像 job 使用最终版本元数据构建并加载一次镜像，再验证 nonroot/只读运行、初始化、健康检查、优雅停止和持久化重启；`CI Gate` 聚合所有结果并作为 main 的唯一 required check。现有 self-hosted 优先、macOS 次选、GitHub-hosted fallback 的 runner 选择策略不变。PR 与 main 不登录仓库、不发布镜像。独立 Release workflow 只接受 main 历史上的 `vMAJOR.MINOR.PATCH`，重新执行代码、观测配置与最终镜像门禁，经 `dockerhub-production` Environment 批准后登录 Docker Hub，把已经测试的同一个本地镜像添加 SemVer 与 `latest` 标签并逐一推送；完整版本标签不可覆盖，所有标签必须解析到同一 digest，digest 附带 SPDX SBOM、keyless Cosign 签名和 GitHub build provenance。BuildKit 的 GHA layer cache 使用 `mode=min`，只导出最终结果所需的最小集合；它只用于跨运行加速，上传失败不影响门禁，也不作为发布物传递或正确性依据。
...
- Chromium 确定性端到端链路把采集投递、管理安全和响应式验证拆为测试级隔离场景，每个场景使用独立临时目录、SQLite 和随机端口，共享一次预编译的 Go harness，并同时运行桌面浅色与 Pixel 7 触控深色项目：覆盖管理员初始化、二维码登录、关注关系与空间基线、综合流采集、历史归档、失败 Outbox、同目录重启、人工重试、无刷新 WebSocket 重连、资源编辑、设置持久化、操作日志安全摘要、秘密不回显，以及密码变更后当前设备获得替代会话、其他旧会话失效；浏览器与 harness 固定使用 `Asia/Shanghai`，前端自托管带 Unicode range 的 Noto Sans SC 可变字体，从而使日期、中文排版和页面高度不依赖 runner 环境；使用 axe 扫描登录、概览、操作日志和移动历史页面，并提交移动历史视觉基线；测试只连接本地 TLS 伪上游；
