...
知识星球匿名提问不会返回提问者身份；归档时使用主题 ID 构造稳定的匿名作者 ID，不能因为缺少 `question.owner` 阻断整页同步。知识星球业务码 `1059` 表示上游拒绝 Cookie 驱动的非官方客户端，必须作为独立、可行动的错误展示，不能伪装成普通网络故障、权限错误或响应结构变化。Cookie 链路不通过伪造客户端特征规避该限制；稳定恢复需要迁移到官方 OAuth/MCP。
