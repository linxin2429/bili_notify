...
wait_for_probe "admin session" --insecure --url https://127.0.0.1:8443/api/v2/session --contains '"setup_required":true'
...
docker exec "${container}" /bili-notify healthcheck \
  --insecure \
  --method POST \
  --url https://127.0.0.1:8443/api/v2/setup \
...
wait_for_probe "admin session after restart" --insecure --url https://127.0.0.1:8443/api/v2/session --contains '"setup_required":false'
