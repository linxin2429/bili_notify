...
func TestApplicationHTTPIntegration(t *testing.T) {
	...
	response = integrationRequest(t, client, http.MethodGet, adminURL+"/api/v4/session", "", nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	...
	response = integrationRequest(t, client, http.MethodPost, adminURL+"/api/v4/setup", "", map[string]any{
	...
	response = integrationRequest(t, client, http.MethodPost, adminURL+"/api/v4/channels", csrf, map[string]any{
	...
	response = integrationRequest(t, client, http.MethodGet, adminURL+"/api/v4/sources?platform=bilibili", "", nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	...
	response = integrationRequest(t, client, http.MethodGet, adminURL+"/api/v4/channels", "", nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	...
	response = integrationRequest(t, client, http.MethodGet, adminURL+"/api/v4/audit-logs?limit=100", "", nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
