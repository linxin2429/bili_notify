...
func TestSetAuthQueuesLifecycleAlerts(t *testing.T) {
	...
	engine.setAuth(t.Context(), true)
	engine.setAuth(t.Context(), false)
	engine.setAuth(t.Context(), true)
	deliveries, err := store.ListDeliveries(0)
	require.NoError(t, err)
...
func TestEngineErrorAndMediaHelpers(t *testing.T) {
	...
	engine.handleBiliAPIError(t.Context(), &bilibili.APIError{Kind: bilibili.ErrorRiskControl, Message: "blocked"})
	assert.Greater(t, engine.riskUntil.Load(), time.Now().Unix())
	engine.authValid.Store(true)
	engine.handleBiliAPIError(t.Context(), &bilibili.APIError{Kind: bilibili.ErrorAuthentication, Message: "expired"})
