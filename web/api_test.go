...
func TestRetryDeliveryAPI(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			request, err := http.NewRequestWithContext(t.Context(), http.MethodPost, httpServer.URL+"/api/v4/deliveries/"+id+"/retry", nil)
			require.NoError(t, err)
			if tt.authenticated {
...
func TestChannelAuditDoesNotPersistSecretValues(t *testing.T) {
	...
	require.NoError(t, err)
	request, err := http.NewRequestWithContext(t.Context(), http.MethodPost, httpServer.URL+"/api/v4/channels", bytes.NewReader(body))
	require.NoError(t, err)
...
func TestLoginAuditSessionCorrelation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			request, err := http.NewRequestWithContext(t.Context(), http.MethodPost, httpServer.URL+"/api/v4/session", bytes.NewReader(body))
			require.NoError(t, err)
			request.Header.Set("Content-Type", "application/json")
...
func TestAuditWriteFailurePreservesBusinessResponse(t *testing.T) {
	...
	request, err := http.NewRequestWithContext(t.Context(), http.MethodDelete, httpServer.URL+"/api/v4/session", nil)
	require.NoError(t, err)
	request.Header.Set("X-CSRF-Token", csrf)
	request.AddCookie(&http.Cookie{Name: sessionCookie, Value: token})
	response, err := httpServer.Client().Do(request)
