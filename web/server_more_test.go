...
func TestAuthenticationHTTPLifecycle(t *testing.T) {
	...
	response := authenticationRequest(t, handler, http.MethodGet, "/api/v2/session", nil, "", "")
	assert.Equal(t, http.StatusOK, response.Code)
	assert.JSONEq(t, `{"setup_required":true,"authenticated":false}`, response.Body.String())

	response = authenticationRequest(t, handler, http.MethodPost, "/api/v2/setup", map[string]string{
	...
	response = authenticationRequest(t, handler, http.MethodGet, "/api/v2/session", nil, token, "")
	assert.Equal(t, http.StatusOK, response.Code)
	assert.Contains(t, response.Body.String(), `"authenticated":true`)
	assert.Contains(t, response.Body.String(), login.CSRF)

	response = authenticationRequest(t, handler, http.MethodPut, "/api/v2/session/password", map[string]string{
	...
	response = authenticationRequest(t, handler, http.MethodGet, "/api/v2/runtime", nil, token, "")
	assertAPIError(t, response, http.StatusUnauthorized, "authentication_required")
	response = authenticationRequest(t, handler, http.MethodDelete, "/api/v2/session", nil, newToken, replacement.CSRF)
	assert.Equal(t, http.StatusNoContent, response.Code)
	require.Len(t, response.Result().Cookies(), 1)
	assert.Equal(t, -1, response.Result().Cookies()[0].MaxAge)

	response = authenticationRequest(t, handler, http.MethodPost, "/api/v2/session", map[string]string{"password": "correct horse battery staple"}, "", "")
	assertAPIError(t, response, http.StatusUnauthorized, "invalid_credentials")
	response = authenticationRequest(t, handler, http.MethodPost, "/api/v2/session", map[string]string{"password": "replacement horse battery staple"}, "", "")
	assert.Equal(t, http.StatusOK, response.Code)
}
...
func TestAuthenticationRateLimit(t *testing.T) {
	...
	handler := server.adminHandler()
	for range 5 {
		response := authenticationRequest(t, handler, http.MethodPost, "/api/v2/session", map[string]string{"password": "wrong"}, "", "")
...
func TestAuthenticationRateLimitIsolationRecoveryAndProxyHeaders(t *testing.T) {
	...
	requestLogin := func(remote, forwardedFor, password string) *httptest.ResponseRecorder {
		...
		request := httptest.NewRequest(http.MethodPost, "/api/v2/session", body)
...
func TestAdminSecurityHeaders(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "HTML", path: "/"},
		{name: "API", path: "/api/v2/session"},
		{name: "missing API", path: "/api/v2/missing"},
	}
...
func TestJSONBodySizeAndEncodingBoundaries(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			request := httptest.NewRequest(http.MethodPost, "/api/v2/ups", bytes.NewReader(tt.body))
...
func TestEncodedAPIPathsCannotEscapeTheirRoute(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "encoded traversal", path: "/api/v2/dynamics/%2e%2e%2fsession"},
		{name: "encoded NUL", path: "/api/v2/dynamics/%00"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
...
func TestIndexAndUnknownAPIRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown API stays JSON", path: "/api/v2/missing", status: http.StatusNotFound, contentType: "application/json; charset=utf-8", contains: `"not_found"`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
