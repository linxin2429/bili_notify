...
export function AppShell() {
  ...
  return <div className="app-shell">
    ...
    <nav className="mobile-nav" aria-label="移动端主导航">{navigation.map(item => <NavLink key={item.path} to={item.path} className={({ isActive }) => `mobile-nav__item${isActive ? ' mobile-nav__item--active' : ''}`}><span aria-hidden="true">{item.icon}</span><small>{item.label}</small></NavLink>)}</nav>
  </div>
}
