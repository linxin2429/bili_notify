...
export const deliverySchema = z.object({
  id: z.string(), kind: z.enum(['dynamic', 'comment']).optional(), dynamic: dynamicDeliveryPreviewSchema.optional(),
...
export const dynamicMediaSchema = z.object({ kind: z.string(), url: z.string(), width: z.number().int().optional(), height: z.number().int().optional() })
export const dynamicStatsSchema = z.object({ forwards: z.number().int(), comments: z.number().int(), likes: z.number().int() })
export const dynamicVideoSchema = z.object({ duration: z.string().optional(), views: z.string().optional(), danmaku: z.string().optional() })
export const dynamicPreviewSchema = z.object({
  ...
  target_url: z.string().optional(), badge: z.string().optional(), media: z.array(dynamicMediaSchema).optional(), video: dynamicVideoSchema.optional(),
})
export const dynamicHistorySchema = z.object({
  ...
  url: z.string().optional(), target_url: z.string().optional(), badge: z.string().optional(), media: z.array(dynamicMediaSchema).optional(),
  stats: dynamicStatsSchema.optional(), video: dynamicVideoSchema.optional(), original: dynamicPreviewSchema.optional(),
})
...
export const cursorPageSchema = <T extends z.ZodType>(item: T) => z.object({
  items: z.array(item),
  page: z.object({ next_cursor: z.string(), has_more: z.boolean() }),
})
...
export const sessionStateSchema = z.object({ setup_required: z.boolean(), authenticated: z.boolean(), csrf_token: z.string().optional() })
