...
export function AuthScreen({ setup }: { setup: boolean }) {
  ...
  return <main className="auth-screen"><Card className="auth-card"><header className="brand"><span className="brand__mark">BN</span><div><h1>Bili Notify</h1><p>{setup ? '完成安全初始化' : '登录实时管理台'}</p></div></header>
    ...
    <div className="form-stack">
      ...
      <TextField label={setup ? '设置管理员密码' : '管理员密码'} type="password" value={password} onChange={setPassword} required autoComplete={setup ? 'new-password' : 'current-password'} description={setup ? '至少 12 个字节' : undefined} />
      ...
      <Button variant="primary" isDisabled={!password || (setup && !code)} busy={mutation.isPending} onPress={submit}>{setup ? '初始化并登录' : '登录'}</Button>
    </div>
  </Card></main>
}
