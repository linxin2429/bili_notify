...
const labels = { queued: '排队中', running: '处理中', succeeded: '已完成', failed: '失败', canceled: '已取消' } as const
...
export function AIWorkbenchPage() {
  ...
  const [kind, setKind] = useState<'transcription' | 'summary'>('transcription'); const [bvid, setBVID] = useState(''); const [page, setPage] = useState('0'); const [text, setText] = useState(''); const [source, setSource] = useState(''); const [profile, setProfile] = useState(''); const [prompt, setPrompt] = useState(''); const [selected, setSelected] = useState('')
  ...
  const action = useMutation({ mutationFn: async ({ id, verb }: { id: string; verb: 'cancel' | 'retry' | 'delete' }) => { if (verb === 'cancel') await resources.cancelAIJob(csrf, id); else if (verb === 'retry') await resources.retryAIJob(csrf, id); else await resources.deleteAIJob(csrf, id) }, onSuccess: () => { notify('任务状态已更新', 'success'); invalidate() }, onError: error => notify(apiErrorMessage(error), 'danger') })
  if (profiles.isPending || prompts.isPending || jobs.isPending || worker.isPending) return <LoadingState label="正在加载 AI 工作台" />
  if (profiles.error || prompts.error || jobs.error || worker.error) return <PageError error={profiles.error || prompts.error || jobs.error || worker.error} />
  ...
  return <div className="page-stack"><PageHeader title="AI 工作台" subtitle="转写和总结由独立 Worker 异步执行，关闭页面不会中断任务。" />
    ...
    <Card><form className="form-stack" onSubmit={event => { event.preventDefault(); submit.mutate({ profileID, promptID }) }}>
      ...
      {!candidates.length || (kind === 'summary' && !prompts.data.length) ? <Alert tone="warning">请先在 AI 设置中补齐模型配置档和提示词。</Alert> : <Button type="submit" variant="primary" busy={submit.isPending}>提交任务</Button>}
    </form></Card>
    <section><h2>任务记录</h2>{!jobs.data.items.length ? <EmptyState title="还没有 AI 任务" icon="✦" /> : <div className="list-stack">{jobs.data.items.map(job => <Card key={job.id} className="ai-job-card"><button type="button" className="ai-job-card__main" onClick={() => setSelected(selected === job.id ? '' : job.id)}><div><div className="card-title-inline"><Badge tone={tone(job)}>{labels[job.state]}</Badge><strong>{job.kind === 'transcription' ? '视频转写' : '文本总结'}</strong></div><p className="muted">{job.stage} · 尝试 {job.attempts} 次</p></div><progress max="100" value={job.progress} aria-label={`任务进度 ${job.progress}%`} /></button>{selected === job.id && <JobDetail job={job} onAction={verb => action.mutate({ id: job.id, verb })} />}</Card>)}</div>}</section>
  </div>
}
