...
function JobDetail({ job, onAction }: { job: AIJob; onAction: (verb: 'cancel' | 'retry' | 'delete') => void }) {
  ...
  return <div className="ai-job-detail">{value.last_error && <Alert tone="danger">{value.error_code && <strong>{value.error_code}：</strong>}{value.last_error}</Alert>}{value.transcription_result && <div className="transcript"><h3>{value.transcription_result.title}</h3>{value.transcription_result.pages.map(item => <section key={item.page}><h3>P{item.page} · {item.title}</h3>{item.segments.map((segment, index) => <p key={`${segment.start_ms}-${index}`}><a target="_blank" rel="noreferrer" href={`https://www.bilibili.com/video/${value.transcription_result?.bvid || ''}?p=${item.page}&t=${Math.floor(segment.start_ms / 1000)}`}>{formatTime(segment.start_ms)}</a><span>{segment.text}</span></p>)}</section>)}</div>}{value.summary_result && <div className="summary-result"><h3>总结结果</h3><pre>{value.summary_result.markdown}</pre></div>}<div className="button-row">{['queued', 'running'].includes(value.state) && <Button danger onPress={() => onAction('cancel')}>取消任务</Button>}{['failed', 'canceled', 'skipped'].includes(value.state) && <Button variant="outline" onPress={() => onAction('retry')}>重新执行</Button>}{['succeeded', 'failed', 'canceled', 'skipped'].includes(value.state) && <Button danger onPress={() => onAction('delete')}>删除记录</Button>}</div></div>
}
