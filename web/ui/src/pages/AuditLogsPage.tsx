...
export function AuditLogsPage({ api, timeZone, refresh }: { api: AdminAPI; timeZone: string; refresh: number }) {
  ...
  return <Stack spacing={3}><PageHeader title="操作日志" subtitle="查询管理员认证、配置变更和手动运维操作；日志不会保存密码、令牌或 Webhook 内容。" />
    <Paper sx={{ p: 2 }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}><FormControl sx={{ minWidth: 190 }}><InputLabel id="audit-action-label">操作</InputLabel><Select labelId="audit-action-label" label="操作" value={action} onChange={event => updateParams({ action: event.target.value || undefined })}><MenuItem value="">全部操作</MenuItem>{auditActions.map(value => <MenuItem key={value} value={value}>{auditActionLabel(value)}</MenuItem>)}</Select></FormControl><FormControl sx={{ minWidth: 140 }}><InputLabel id="audit-outcome-label">结果</InputLabel><Select labelId="audit-outcome-label" label="结果" value={outcome} onChange={event => updateParams({ outcome: event.target.value || undefined })}><MenuItem value="">全部结果</MenuItem><MenuItem value="success">成功</MenuItem><MenuItem value="failure">失败</MenuItem><MenuItem value="denied">已拒绝</MenuItem></Select></FormControl><TextField label="资源、IP 或请求 ID" value={draftQ} onChange={event => setDraftQ(event.target.value)} fullWidth /><TextField label="开始时间" type="datetime-local" value={from} onChange={event => updateParams({ from: event.target.value || undefined })} InputLabelProps={{ shrink: true }} sx={{ minWidth: 210 }} /><TextField label="结束时间" type="datetime-local" value={to} onChange={event => updateParams({ to: event.target.value || undefined })} InputLabelProps={{ shrink: true }} sx={{ minWidth: 210 }} /></Stack></Paper>
