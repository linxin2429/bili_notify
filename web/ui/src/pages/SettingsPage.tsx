...
function SettingsEditor({ initial }: { initial: RuntimeSettings }) {
  ...
  return <div className="page-stack"><PageHeader title="设置" subtitle="运行参数由服务端持久化；主题偏好只保存在当前浏览器。" />
    ...
    <SettingsSection title="修改管理员密码" description="修改成功后，其他设备的会话会立即失效，当前设备会自动更新会话。"><div className="form-stack form-narrow"><TextField label="当前密码" type="password" value={current} onChange={setCurrent} autoComplete="current-password" /><TextField label="新密码" type="password" value={replacement} onChange={setReplacement} autoComplete="new-password" description="至少 12 个字节" /><TextField label="确认新密码" type="password" value={confirm} onChange={setConfirm} autoComplete="new-password" /><Button variant="primary" busy={password.isPending} isDisabled={!current || !replacement} onPress={changePassword}>修改密码</Button></div></SettingsSection>
  </div>
}
