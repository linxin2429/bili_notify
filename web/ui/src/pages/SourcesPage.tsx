...
function SourceSection({ title, empty, action, sources, edit, remove, timeZone }: { title: string; empty: string; action: React.ReactNode; sources: Source[]; edit: (source: Source) => void; remove: (source: Source) => void; timeZone: string }) {
  return <section className="page-stack"><div className="card-title"><h2>{title}</h2>{action}</div>{sources.length === 0 ? <EmptyState icon={<Database />} title={empty} /> : <div className="card-grid">{sources.map(source => <Card key={source.id}><div className="card-title"><div><h2>{source.note || source.name}</h2><p>{source.name} · {source.external_id}</p></div><Badge tone={source.enabled ? 'success' : 'neutral'}>{source.enabled ? '已启用' : '已停用'}</Badge></div><div className="badge-row"><Badge>{baselineLabel(source.baseline_state)}</Badge>{source.owner_name && <Badge tone="info">星主 {source.owner_name}</Badge>}{source.platform === 'zsxq' && <Badge tone="info">{source.zsxq_topic_mode === 'selected_authors' ? `指定 ${source.zsxq_authors?.length || 0} 位作者` : '全部主题'}</Badge>}<Badge>已回补 {source.backfill_done}</Badge>{source.consecutive_fails > 0 && <Badge tone="danger">连续失败 {source.consecutive_fails}</Badge>}</div>{source.platform === 'zsxq' && source.zsxq_topic_mode === 'selected_authors' && <p className="muted">作者 {(source.zsxq_authors || []).map(author => author.name ? `${author.name}（${author.user_id}）` : author.user_id).join('、')}</p>}{source.last_success_at && <p className="muted">最近成功 {formatDate(source.last_success_at, timeZone)}</p>}{source.last_error && <Alert tone="danger">{source.last_error}</Alert>}<div className="button-row"><Button onPress={() => edit(source)}><Pencil aria-hidden="true" />编辑</Button><Button danger onPress={() => remove(source)}><Trash2 aria-hidden="true" />删除</Button></div></Card>)}</div>}</section>
}

type BilibiliSourceDraft = { externalID: string; name: string; note: string; enabled: boolean }
type TopicMode = 'all' | 'selected_authors'
type AuthorDraft = { userID: string; name: string }
type ZSXQSourceDraft = { groupID: string; note: string; enabled: boolean; topicMode: TopicMode; authors: AuthorDraft[] }
type EditSourceDraft = { id: string; platform: Source['platform']; name: string; note: string; enabled: boolean; topicMode?: TopicMode; authors: AuthorDraft[] }

function cleanAuthors(authors: AuthorDraft[]) { return authors.map(author => ({ user_id: author.userID.trim(), ...(author.name.trim() ? { name: author.name.trim() } : {}) })) }
function authorsForMode(mode: TopicMode, authors: AuthorDraft[]) { return mode === 'selected_authors' ? cleanAuthors(authors) : [] }
function validAuthors(mode: TopicMode, authors: AuthorDraft[]) { return mode === 'all' || (authors.length > 0 && authors.every(author => /^[1-9][0-9]*$/.test(author.userID.trim())) && new Set(authors.map(author => author.userID.trim())).size === authors.length) }
...
function ZSXQSourceDialog({ groups, existing, loading, loadError, busy, error, onRetry, onClose, onSave }: { groups: ZSXQGroup[]; existing: Source[]; loading: boolean; loadError: string; busy: boolean; error: string; onRetry: () => void; onClose: () => void; onSave: (value: ZSXQSourceDraft) => void }) {
  const [groupID, setGroupID] = useState(''); const [note, setNote] = useState(''); const [enabled, setEnabled] = useState(true); const [topicMode, setTopicMode] = useState<TopicMode>('all'); const [authors, setAuthors] = useState<AuthorDraft[]>([])
  ...
  return <Dialog open onClose={onClose} title="添加知识星球采集源" actions={<><Button onPress={onClose}>取消</Button><Button variant="primary" busy={busy} isDisabled={!groupID || loading || Boolean(loadError) || !validAuthors(topicMode, authors)} onPress={() => onSave({ groupID, note, enabled, topicMode, authors })}>保存</Button></>}><div className="form-stack">{error && <Alert tone="danger">{error}</Alert>}{loading ? <LoadingState label="正在读取账号中的星球" /> : loadError ? <Alert tone="danger"><p>{loadError}</p><Button variant="outline" onPress={onRetry}>重试</Button></Alert> : groups.length === 0 ? <Alert>当前账号没有可选择的星球。</Alert> : <><SelectField label="星球" value={groupID} onChange={setGroupID} options={options} />{!available && <p className="muted">账号中的星球都已经添加为采集源。</p>}<TopicFilterFields mode={topicMode} setMode={setTopicMode} authors={authors} setAuthors={setAuthors} /><TextField label="备注" value={note} onChange={setNote} /><SwitchField checked={enabled} onChange={setEnabled}>启用采集</SwitchField></>}</div></Dialog>
}
...
function EditSourceDialog({ value, busy, error, onClose, onSave }: { value: Source; busy: boolean; error: string; onClose: () => void; onSave: (value: EditSourceDraft) => void }) {
  const [name, setName] = useState(value.name); const [note, setNote] = useState(value.note || ''); const [enabled, setEnabled] = useState(value.enabled); const [topicMode, setTopicMode] = useState<TopicMode>(value.zsxq_topic_mode || 'all'); const [authors, setAuthors] = useState<AuthorDraft[]>((value.zsxq_authors || []).map(author => ({ userID: author.user_id, name: author.name || '' })))
  return <Dialog open onClose={onClose} title="编辑采集源" actions={<><Button onPress={onClose}>取消</Button><Button variant="primary" busy={busy} isDisabled={!name.trim() || (value.platform === 'zsxq' && !validAuthors(topicMode, authors))} onPress={() => onSave({ id: value.id, platform: value.platform, name, note, enabled, topicMode: value.platform === 'zsxq' ? topicMode : undefined, authors: value.platform === 'zsxq' ? authors : [] })}>保存</Button></>}><div className="form-stack">{error && <Alert tone="danger">{error}</Alert>}<TextField label={value.platform === 'zsxq' ? '星球 ID' : 'UID'} value={value.external_id} onChange={() => undefined} disabled /><TextField label="来源名称" value={name} onChange={setName} required />{value.platform === 'zsxq' && <TopicFilterFields mode={topicMode} setMode={setTopicMode} authors={authors} setAuthors={setAuthors} />}<TextField label="备注" value={note} onChange={setNote} /><SwitchField checked={enabled} onChange={setEnabled}>启用采集</SwitchField></div></Dialog>
}
