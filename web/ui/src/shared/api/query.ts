...
export const queries = {
  ...
  aiStatus: (live = false) => queryOptions({ queryKey: queryKeys.aiStatus, queryFn: ({ signal }) => resources.aiStatus(signal), staleTime: 5_000, refetchInterval: live ? false : 10_000 }),
  ...
  aiJobs: (query: { kind?: string; state?: string; limit?: number; offset?: number } = {}, live = false) => queryOptions({ queryKey: queryKeys.aiJobs(query), queryFn: ({ signal }) => resources.aiJobs(query, signal), staleTime: 2_000, refetchInterval: live ? false : 5_000 }),
