...
describe('presentation helpers', () => {
  ...
  it('normalizes preview bodies and media URLs', () => {
    ...
    expect(historyMediaURL('https://example.com/a.jpg', 240)).toBe('https://example.com/a.jpg'); expect(historyMediaURL('/api/v4/dynamics/1/media/0', 240)).toBe('/api/v4/dynamics/1/media/0'); expect(historyMediaURL(' ', 240)).toBe(''); expect(historyMediaURL('bad url', 0)).toBe('bad url')
  })
  ...
  it('formats delivery and audit variants', () => {
    ...
    const comment = makeDelivery({ kind: 'comment', content_id: 'BV', title: '评论回复', summary: '内容摘要' })
    expect(deliveryTitle(comment)).toBe('评论回复'); expect(deliverySummary(comment)).toBe('内容摘要')
